...
# --- CSS ADAPTIF (SUPORT LIGHT & DARK MODE) ---
st.markdown("""
    <style>
...
day_type = st.radio("Pilih tipe hari:", ["Weekday", "Weekend"], label_visibility="collapsed")
...
c1, c2 = st.columns(2)
work_hours = c1.number_input("Jam Kerja (Total)", 0.0, 24.0, 8.0)
screen_time = c2.number_input("Waktu Layar (Screen Time)", 0.0, 24.0, 6.0)
...
c3, c4 = st.columns(2)
meetings = c3.slider("Jumlah Pertemuan/Meeting", 0, 20, 2)
breaks = c4.slider("Frekuensi Istirahat Sejenak", 0, 10, 3)
...
c5, c6 = st.columns(2)
after_hours = c5.number_input("Jam Lembur/Setelah Jam Kantor", 0.0, 10.0, 0.0)
app_switches = c6.slider("Tingkat Perpindahan Aplikasi (Multitasking)", 1, 100, 20)
...
c7, c8 = st.columns(2)
sleep_hours = c7.number_input("Lama Tidur (Jam)", 0.0, 12.0, 7.0)
fatigue_score = c8.slider("Skor Kelelahan Fisik", 1.0, 10.0, 5.0)
...
c9, c10 = st.columns(2)
task_completion = c9.slider("Persentase Tugas Selesai", 0, 100, 80)
isolation_index = c10.slider("Indeks Perasaan Terisolasi", 1, 10, 3)

# --- PROSES ANALISIS ---
st.divider()
if st.button("Lihat Hasil Tes", use_container_width=True):
    with st.spinner("Sedang mengkalkulasikan ceritamu..."):
        time.sleep(2) 
        ...
        if prediction == 0:
            ...
            st.info("Kamu memiliki manajemen diri yang sangat baik. Pertahankan ritme ini ya.")
        elif prediction == 1:
